...
class Sketch(CanvasBase):
    ...
    def set_pose1(self):
        """Pose 1: A slight bend in all legs."""
        for i in range(8):  # Loop through each leg
            leg_name = f"leg{i + 1}"
            self.cDict[leg_name].setCurrentAngle(15, self.cDict[leg_name].uAxis)  # Slight bend in the first joint of each leg
            self.cDict[leg_name].children[0].setCurrentAngle(30, self.cDict[leg_name].vAxis)  # Bend at second joint
            self.cDict[leg_name].children[0].children[0].setCurrentAngle(-15, self.cDict[leg_name].wAxis)  # Slight reverse at third joint

    def set_pose2(self):
        """Pose 2: Front four legs raised, rear four legs bent."""
        for i in range(4):  # Front four legs in raised stance
            leg_name = f"leg{i + 1}"
            self.cDict[leg_name].setCurrentAngle(45, self.cDict[leg_name].uAxis)  # Raise the front legs
            self.cDict[leg_name].children[0].setCurrentAngle(30, self.cDict[leg_name].vAxis)
            self.cDict[leg_name].children[0].children[0].setCurrentAngle(30, self.cDict[leg_name].wAxis)
        for i in range(4, 8):  # Rear four legs bent
            leg_name = f"leg{i + 1}"
            self.cDict[leg_name].setCurrentAngle(-20, self.cDict[leg_name].uAxis)
            self.cDict[leg_name].children[0].setCurrentAngle(-30, self.cDict[leg_name].vAxis)

    def set_pose3(self):
        """Pose 3: Alternating leg movement."""
        for i in range(4):  # Every other leg moved forward
            leg_name = f"leg{i * 2 + 1}"
            self.cDict[leg_name].setCurrentAngle(45, self.cDict[leg_name].uAxis)  # Move forward
        for i in range(4):  # The alternate legs moved backward
            leg_name = f"leg{i * 2 + 2}"
            self.cDict[leg_name].setCurrentAngle(-45, self.cDict[leg_name].uAxis)  # Move backward

    def set_pose4(self):
        """Pose 4: Fold all legs under."""
        for i in range(8):  # All legs folded under
            leg_name = f"leg{i + 1}"
            self.cDict[leg_name].setCurrentAngle(-45, self.cDict[leg_name].uAxis)  # Fold legs in

    def set_pose5(self):
        """Pose 5: Front legs raised, rear legs extended."""
        for i in range(4):  # Front four legs raised
            leg_name = f"leg{i + 1}"
            self.cDict[leg_name].setCurrentAngle(60, self.cDict[leg_name].uAxis)  # Front legs raised
        for i in range(4, 8):  # Rear four legs extended
            leg_name = f"leg{i + 1}"
            self.cDict[leg_name].setCurrentAngle(-60, self.cDict[leg_name].uAxis)  # Rear legs extended
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if chr(keycode).lower() == "1":
            self.set_pose1()  # Trigger Pose 1
            self.update()

        if chr(keycode).lower() == "2":
            self.set_pose2()  # Trigger Pose 2
            self.update()

        if chr(keycode).lower() == "3":
            self.set_pose3()  # Trigger Pose 3
            self.update()

        if chr(keycode).lower() == "4":
            self.set_pose4()  # Trigger Pose 4
            self.update()

        if chr(keycode).lower() == "5":
            self.set_pose5()  # Trigger Pose 5
            self.update()
